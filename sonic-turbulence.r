...
avgStats <- function(FilePath, granularity = "30mins", st_to = NULL, 
  ...
  ######################################################################################
  #####                                                                            #####
  #####                       Written by Christoph Häni                            #####
  #####    https://github.com/hafl-gel/gel-scripts/blob/main/sonic-turbulence.r    #####
  #####                                                                            #####
  ######################################################################################
  ######################################################################################
...
calcg <- function(lat=NULL, h=NULL){ # calculate the local gravitational acceleration
##########################################
##########################################
#####                                #####
#####    Written by Marcel Bühler    #####
#####                                #####
##########################################
##########################################
    IGF <- 9.780327 * (1 + 0.0053024*sin(lat * pi/180)^2 - 0.0000058*sin(2*lat * pi/180)^2)
    FAC <- -3.086 * 10^-6 * h
    g <- IGF + FAC
    g
}

wind_statistics <- function(wind,z_canopy,z_sonic,latitude,elevation){
  ######################################################################################
  ######################################################################################
  #####                                                                            #####
  #####                       Written by Christoph Häni                            #####
  #####    https://github.com/hafl-gel/gel-scripts/blob/main/sonic-turbulence.r    #####
  #####                    The function was adapted by Marcel Bühler               #####
  #####                                                                            #####
  ######################################################################################
  ######################################################################################
  ...
  suppressWarnings(Ustar <- c((Cov_sonic["<u'w'>"]^2+Cov_sonic["<v'w'>"]^2)^0.25,use.names = FALSE)) # change to more correct formula
  T_K <- mean(wind$Tm + wind$Tp)
  U <- mean(wind$um + wind$up)
  L <- c(-Ustar^3 * T_K / (0.4 * calcg(lat=latitude,h=elevation) * Cov_sonic["<w'T'>"]),use.names = FALSE) # use local g
  if(!is.na(z_canopy)){
    d <- 2/3 * z_canopy
...
evalSonic_AU <- function(
  ######################################################################################
  ######################################################################################
  #####                                                                            #####
  #####                       Written by Christoph Häni                            #####
  #####    https://github.com/hafl-gel/gel-scripts/blob/main/sonic-turbulence.r    #####
  #####                    The function was adapted by Marcel Bühler to work       #####
  #####    with the input data of the sonic anemometers from Aarhus University     #####
  #####                                                                            #####
  ######################################################################################
  ######################################################################################
    file_directory = NULL
    ...
    , z_sonic = NULL
    , dev_north = 0
    , detrending_method = c(u = "blockAVG", v = "blockAVG", w = "blockAVG", T = "linear") # blockAVG/linear/linear_robust/ma_xx (xx = time in seconds)
...
# browser()
  ...
  } else if(deparse(substitute(rawdata_function)) == "readSonicEVS_csv ") {
  ...
  } else if(deparse(substitute(rawdata_function)) == "readSonicAU_txt") {
    dailyfiles <- dir(file_directory, pattern = '^Flavia-\\d{2}-\\d{2}-\\d{4}_\\d{2}-\\d{2}\\.txt$', recursive = TRUE)
    if(length(dailyfiles) == 0){
      stop("Directory '",file_directory,"' doesn't have any sonic files matching provided 'file_pattern_SonicAU_txt'!")
    }
    starttimes <- fast_strptime(gsub('.*-(\\d{2}-\\d{2}-\\d{4}_\\d{2}-\\d{2})\\.txt', "\\1",dailyfiles),format="%d-%m-%Y_%H-%M", tz=tz_sonic, lt=FALSE)
  } else if(deparse(substitute(rawdata_function)) == "readSonicAU2_txt") {
    dailyfiles <- dir(file_directory, pattern = '^SonicInt_\\d{8}_\\d{4}\\.txt$', recursive = TRUE)
    if(length(dailyfiles) == 0){
      stop("Directory '",file_directory,"' doesn't have any sonic files matching provided 'file_pattern_SonicAU_txt'!")
  }
    starttimes <- fast_strptime(gsub("^SonicInt_(\\d{8})_(\\d{4})\\.txt$","\\1_\\2",dailyfiles),format="%Y%m%d_%H%M", tz=tz_sonic, lt=FALSE)
    } else {
NA
  }
  ...
  } else if(length(z_canopy) == 2){
    z_canopy <- as.ibts(data.frame(canopy=seq(z_canopy[1],z_canopy[2],length.out=Out[,.N])),st=Out[,start_interval],et=Out[,end_interval])
    tzone(z_canopy) <- tz_sonic
    z_canopy <- pool(z_canopy,st.to = Out[,format(start_interval)],et.to = Out[,format(end_interval)])[,1,drop=TRUE,keepAtts=FALSE]
  } else {
  ...
  Out[,c("z_canopy","d","Ustar","L","z0","sUu","sVu","sWu","T_sonic","U_sonic","up.up","vp.vp","wp.wp","Tp.Tp","up.vp","up.wp","up.Tp","vp.wp","vp.Tp","wp.Tp")] <- Data[,{
    WS <- wind_statistics(.(up=up,vp=vp,wp=wp,Tp=Tp,um=um,Tm=Tm),z_canopy[as.character(.BY[[1]])],z_sonic,latitude=latitude,elevation=elevation)
    .(
  ...
  Out[,paste0("sub_",c("z_canopy","d","Ustar","L","z0","sUu","sVu","sWu","up.up","vp.vp","wp.wp","Tp.Tp","up.vp","up.wp","up.Tp","vp.wp","vp.Tp","wp.Tp"))] <- Data[,{
    # browser()
    WS <- wind_statistics(.(up=sub_up,vp=sub_vp,wp=sub_wp,Tp=sub_Tp,um=sub_um,Tm=sub_Tm),z_canopy[as.character(BinPeriod[1])],z_sonic,latitude=latitude,elevation=elevation)
    .(
